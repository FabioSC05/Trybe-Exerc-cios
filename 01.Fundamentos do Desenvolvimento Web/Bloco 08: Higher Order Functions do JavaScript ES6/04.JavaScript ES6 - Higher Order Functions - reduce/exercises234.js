...
function reduceNames(array) {
  return array.reduce((acc, element) => {
    if (typeof acc === 'object') {
      return `${acc.author.name}, ${element.author.name}`;
    }
...
function averageAge(array) {
  const value = array.reduce((acc, element) => {
    if (typeof acc === 'object') {
      return (acc.releaseYear - acc.author.birthYear) + (element.releaseYear - element.author.birthYear);
    }
